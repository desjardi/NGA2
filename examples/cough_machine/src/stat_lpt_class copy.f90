...
module stat_lpt_class
  ...
  public :: stat_plane_lpt

  !> Definition of a stat plane for lpt at a given x,y,z location
  type :: stat_plane_lpt
    ...
    real(WP)            :: xpos,ypos,zpos                                  !< x,y,z position of the particle as it passes through the plane
    integer(kind=8)     :: ID                                              !< Tagged particle ID
    class(lpt), pointer :: lp                                              !< Particle variable
     
    ! Stat collection parameters 
    real(WP) :: dr                                                         !< Radial mesh spacing
    real(WP) :: dd                                                         !< Drop diameter bin spacing
    real(WP) :: Delta_t                                                    !< Time current plane has had stats collected for
    ...
    integer  :: nd                                                         !< Number of bins for diameter
    real(WP), dimension(:), allocatable     :: r                           !< Drop bins
    real(WP), dimension(:,:), allocatable   :: Deltat_r                    !< Time current radial bin has collected for, 0 if no particle has passed
    real(WP), dimension(:,:), allocatable   :: np_r                        !< Number of particles that have passed through the radial bin
    real(WP), dimension(:,:,:), allocatable :: stat,buf                    !< Stat variables and buffers
    character(len=str_medium)                            :: plane_pos      !< Axes that plane lies on
    ...
    logical :: amIn                                                        !< Check to ensure loop is in bounds
  ...
  contains
    ...
    ! procedure :: write  => stat_1d_lpt_write                               !< Write particle stats to files
  end type stat_plane_lpt

  !> Declare lpt_stat constrcuctor 
  interface stat_plane_lpt
    procedure constructor
  end interface stat_plane_lpt
    ...
    case ('x')
      if (this%lp%p(n)%pos(1).ge.this%location.and.this%lp%p(n)%pos(1)-dt*this%lp%p(n)%vel(1).lt.this%location.or. &
        ...
        if (i.le.this%nd) then
          ...
          ! Add posistions
          buf(i,9)=buf(i,5)+this%lp%p(n)%pos(1)
          buf(i,10)=buf(i,6)+this%lp%p(n)%pos(2)
          buf(i,11)=buf(i,7)+this%lp%p(n)%pos(3)
        end if
