...
contains
   ...
   subroutine get_drhoSCdt(this,drhoSCdt,RHO,RHOold,rhoU,rhoV,rhoW)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               FX(i,j,k)=-rhoU(i,j,k)*sum(this%itp_x(:,i,j,k)*SCmid(i-1:i,j,k))+sum(this%itp_x(:,i,j,k)*this%diff(i-1:i,j,k))*sum(this%grd_x(:,i,j,k)*SC_(i-1:i,j,k))
               FY(i,j,k)=-rhoV(i,j,k)*sum(this%itp_y(:,i,j,k)*SCmid(i,j-1:j,k))+sum(this%itp_y(:,i,j,k)*this%diff(i,j-1:j,k))*sum(this%grd_y(:,i,j,k)*SC_(i,j-1:j,k))
               FZ(i,j,k)=-rhoW(i,j,k)*sum(this%itp_z(:,i,j,k)*SCmid(i,j,k-1:k))+sum(this%itp_z(:,i,j,k)*this%diff(i,j,k-1:k))*sum(this%grd_z(:,i,j,k)*SC_(i,j,k-1:k))
            end do
         end do
      end do
   ...
   subroutine solve_implicit(this,dt,resSC,RHO,RHOold,rhoU,rhoV,rhoW)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*(this%div_x(+1,i,j,k)*rhoU(i+1,j,k)*this%itp_x(-1,i+1,j,k)+&
               &                                                         this%div_x( 0,i,j,k)*rhoU(i  ,j,k)*this%itp_x( 0,i  ,j,k)+&
               &                                                         this%div_y(+1,i,j,k)*rhoV(i,j+1,k)*this%itp_y(-1,i,j+1,k)+&
               &                                                         this%div_y( 0,i,j,k)*rhoV(i,j  ,k)*this%itp_y( 0,i,j  ,k)+&
               &                                                         this%div_z(+1,i,j,k)*rhoW(i,j,k+1)*this%itp_z(-1,i,j,k+1)+&
               &                                                         this%div_z( 0,i,j,k)*rhoW(i,j,k  )*this%itp_z( 0,i,j,k  ))*sRHO(i  ,j,k)/(sRHO(i  ,j,k)+sRHOold(i  ,j,k)*(1.0_WP-this%theta)/this%theta)
               this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)+dt*(this%div_x(+1,i,j,k)*rhoU(i+1,j,k)*this%itp_x( 0,i+1,j,k))*sRHO(i+1,j,k)/(sRHO(i+1,j,k)+sRHOold(i+1,j,k)*(1.0_WP-this%theta)/this%theta)
               this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)+dt*(this%div_x( 0,i,j,k)*rhoU(i  ,j,k)*this%itp_x(-1,i  ,j,k))*sRHO(i-1,j,k)/(sRHO(i-1,j,k)+sRHOold(i-1,j,k)*(1.0_WP-this%theta)/this%theta)
               this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)+dt*(this%div_y(+1,i,j,k)*rhoV(i,j+1,k)*this%itp_y( 0,i,j+1,k))*sRHO(i,j+1,k)/(sRHO(i,j+1,k)+sRHOold(i,j+1,k)*(1.0_WP-this%theta)/this%theta)
               this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)+dt*(this%div_y( 0,i,j,k)*rhoV(i,j  ,k)*this%itp_y(-1,i,j  ,k))*sRHO(i,j-1,k)/(sRHO(i,j-1,k)+sRHOold(i,j-1,k)*(1.0_WP-this%theta)/this%theta)
               this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)+dt*(this%div_z(+1,i,j,k)*rhoW(i,j,k+1)*this%itp_z( 0,i,j,k+1))*sRHO(i,j,k+1)/(sRHO(i,j,k+1)+sRHOold(i,j,k+1)*(1.0_WP-this%theta)/this%theta)
               this%implicit%opr(7,i,j,k)=this%implicit%opr(7,i,j,k)+dt*(this%div_z( 0,i,j,k)*rhoW(i,j,k  )*this%itp_z(-1,i,j,k  ))*sRHO(i,j,k-1)/(sRHO(i,j,k-1)+sRHOold(i,j,k-1)*(1.0_WP-this%theta)/this%theta)
            end do
         end do
      end do
      do k=this%cfg%kmin_,this%cfg%kmax_
