 ...
 contains
    ...
    subroutine simulation_init
       ...
       create_and_initialize_flow_solver: block
          ...
          ! Original thing to revert back to just in case
          call param_read('Shock location',xshock)
          ! ! Use shock relations to get post-shock numbers (1)
          ! GP0 = Pref
          ! GP1 = Pref * (2.0_WP*gamm_g*Mag**2 - (gamm_g-1.0_WP)) / (gamm_g+1.0_WP)
          ! Grho1 = Grho0 * (Mag**2 * (gamm_g+1.0_WP) / ((gamm_g-1.0_WP)*Mag**2 + 2.0_WP))
          ! ! Calculate post-shock Mach number
          ! Ma1 = sqrt(((gamm_g-1.0_WP)*(Mag**2)+2.0_WP)/(2.0_WP*gamm_g*(Mag**2)-(gamm_g-1.0_WP)))
          ! ! Calculate post-shock velocity
          ! vshock = -Ma1 * sqrt(gamm_g*GP1/Grho1) + Mag*sqrt(gamm_g*Pref/Grho0)
          ...
          ! Grho1 = 1.0_WP; Ma = Mag;
          ! Grho0 = Grho1 * ((gamm_g-1.0_WP)*(Ma**2) + 2.0_WP) / ((Ma**2) * (gamm_g+1.0_WP))
          ! GP0   = (1.0_WP**2)*Grho0 / (gamm_g*(Ma**2))
