...
module geometry
   ...
   type(config), public :: cfg3 !< This is the spray flow config
...
contains
   ...
   subroutine geometry_init
         ...
         do k=cfg1%kmino_,cfg1%kmaxo_
            do j=cfg1%jmino_,cfg1%jmaxo_
               do i=cfg1%imino_,cfg1%imaxo_
                  ...
                  ! first nozzle wall
                  !if (swh(1,i).le.rad.and.rad.lt.swh(2,i)) cfg1%VF(i,j,k)=0.0_WP
                  if (rad.lt.swh(2,i)) cfg1%VF(i,j,k)=0.0_WP  !< Carve out entire liquid needle
                  ! second nozzle wall
                  if (swh(3,i).le.rad.and.rad.lt.swh(4,i)) cfg1%VF(i,j,k)=0.0_WP
      ...
      ! Create third grid from input params
      create_grid3: block
         use sgrid_class, only: cartesian
         integer :: i,j,k,nx,ny,nz
         real(WP) :: Lx,Ly,Lz,xmax
         ...
         ! General serial grid object with overlap=2 for lpt
         grid3=sgrid(coord=cartesian,no=2,x=x,y=y,z=z,xper=.false.,yper=.false.,zper=.false.,name='spray_region')
      end block create_grid3
      ...
      ! Create masks for config3
      create_spray_region: block
         use messager, only: warn
         integer :: i,j,k
         real(WP) :: xloc,rad,run,rise,hyp
         real(WP), dimension(:,:), allocatable :: swh
         ...
         ! Basic check
         if (cfg3%xm(cfg3%imin  ).lt.xinj_dist+0.5_WP*inj_norm_diam) call warn('[geometry init] cfg3 extends beyond the nozzle ports!')
         if (cfg3%x (cfg3%imax+1).lt.0.0_WP) call warn('[geometry init] cfg3 does not extend beyond the nozzle tip!')
         
         ! Interpolate CAD on our mesh
         allocate(swh(4,cfg3%imino_:cfg3%imaxo_)); swh=0.0_WP
         do i=cfg3%imino_,cfg3%imaxo_
            xloc=cfg3%xm(i)
            ! Skip if past nozzle exit on the right
            if (xloc.gt.0.0_WP) cycle
            ! Skip if past nozzle backend on the left
            if (cfg3%xm(i).lt.nozzle_end) swh(:,i)=0.0_WP
            ! Interpolate switch height
            call bisection(xloc,k,x_swh1,   2)
            swh(1,i)=(xloc-x_swh1(k))/(x_swh1(k+1)-x_swh1(k))*swh1(k+1)+(x_swh1(k+1)-xloc)/(x_swh1(k+1)-x_swh1(k))*swh1(k)
            call bisection(xloc,k,x_swh2, 890)
         ...
         ! Write VF array - includes the overlap here
         cfg3%VF=1.0_WP
         do k=cfg3%kmino_,cfg3%kmaxo_
            do j=cfg3%jmino_,cfg3%jmaxo_
               do i=cfg3%imino_,cfg3%imaxo_
                  rad=sqrt(cfg3%ym(j)**2+cfg3%zm(k)**2)
                  ! Skip if past nozzle exit
                  if (cfg3%xm(i).gt.0.0_WP) cycle
                  ! Skip if past nozzle end
                  if (cfg3%xm(i).lt.nozzle_end) then
                     cfg3%VF(i,j,k)=0.0_WP
                     cycle
                  end if
                  ! first nozzle wall
                  !if (swh(1,i).le.rad.and.rad.lt.swh(2,i)) cfg3%VF(i,j,k)=0.0_WP
                  if (rad.lt.swh(2,i)) cfg3%VF(i,j,k)=0.0_WP  !< Carve out entire liquid needle
                  ! second nozzle wall
                  if (swh(3,i).le.rad.and.rad.lt.swh(4,i)) cfg3%VF(i,j,k)=0.0_WP
                  ! Carve out injector ports
                  if (swh(3,i).le.rad.and.rad.lt.swh(4,i)) then
                     ! Check injector x-y plane
                     hyp =norm2([cfg3%xm(i),cfg3%ym(j),cfg3%zm(k)]-[xinj_dist,0.0_WP,0.0_WP])
                     rise=abs(cfg3%zm(k))
                     run =sqrt(hyp**2-rise**2)
                     if (run.le.inj_norm_diam/2.0_WP) cfg3%VF(i,j,k)=1.0_WP
                     ! Check injector x-z plane
                     hyp =norm2([cfg3%xm(i),cfg3%ym(j),cfg3%zm(k)]-[xinj_dist,0.0_WP,0.0_WP])
                     rise=abs(cfg3%ym(j))
                     run =sqrt(hyp**2-rise**2)
