...
module simulation
   ...
   type(monitor) :: mfile,cflfile,lptfile,khfile
...
contains
   ...
   !> Function to define the base flow
   function base_flow(y) result(U)
      real(WP), intent(in) :: y
      real(WP) :: U
      U = 0.5_WP*Udiff*tanh(y/(0.5_WP*delta))
   end function base_flow

   !> Function to compute the KE of the perturbations
   function get_KEp() result(KEp)
      ...
      do k=fs%cfg%kmino_,fs%cfg%kmaxo_
         do j=fs%cfg%jmino_,fs%cfg%jmaxo_
            do i=fs%cfg%imino_,fs%cfg%imaxo_
               up = fs%U(i,j,k) - base_flow(fs%cfg%ym(j))
               vp = fs%V(i,j,k) 
               KEp = KEp + 0.5_WP * (up*up + vp*vp) * fs%cfg%dx(i) * fs%cfg%dy(j)
            end do
         end do
      end do
      call MPI_ALLREDUCE(MPI_IN_PLACE,KEp,1,MPI_REAL_WP,MPI_SUM,fs%cfg%comm,ierr)
   end function get_KEp
   ...
   subroutine simulation_init
         ...
         do k=fs%cfg%kmino_,fs%cfg%kmaxo_
            do j=fs%cfg%jmino_,fs%cfg%jmaxo_
               do i=fs%cfg%imino_,fs%cfg%imaxo_
                  ...
                  do nX=1,nwaveX
                     do nZ=1,nwaveZ
                        perturb=perturb+wampX(nX)*cos(wnumbX(nX)*(fs%cfg%xm(i)-wshiftX(nX)))*wampZ(nZ)*cos(wnumbZ(nZ)*(fs%cfg%zm(k)-wshiftZ(nZ)))
                     end do
                  end do
                  fs%U(i,j,k)=base + perturb
               end do
            end do
         end do
      ...
      create_monitor: block
         ...
         ! ! KH monitor
         KEp = get_KEp()
         khfile=monitor(fs%cfg%amRoot,'KH')
         call khfile%add_column(time%t,'Time')
         call khfile%add_column(KEp,'KE Perturb')
         call khfile%write()
