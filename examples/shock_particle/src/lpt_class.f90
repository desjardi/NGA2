...
module lpt_class
   ...
   type :: part
      ...
      real(WP) :: T                        !< Particle temperature
...
contains
   ...
   subroutine substep_rk4(this,stage,dt,U,V,W,rho,visc,T,C,stress_x,stress_y,stress_z,heat_flux,srcU,srcV,srcW,srcI)
      ...
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: C         !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      ...
      if (present(srcI)) then
         srcI=srcI/this%cfg%vol; call this%cfg%syncsum(srcI); call this%filter(srcI)
      end if
   ...
   subroutine get_rhs(this,U,V,W,rho,visc,T,C,stress_x,stress_y,stress_z,heat_flux,p,dxdt,dudt,dwdt,dTdt)
      ...
      interpolate: block
         ...
         ! Interpolate the fluid phase temperature
         fT=this%cfg%get_scalar(pos=p%pos,i0=p%ind(1),j0=p%ind(2),k0=p%ind(3),S=T,bc='n')
         ! Interpolate the fluid phase sound speed
         fC=this%cfg%get_scalar(pos=p%pos,i0=p%ind(1),j0=p%ind(2),k0=p%ind(3),S=C,bc='n')
