...
contains
   ...
   subroutine rhs(this,VF,VFU,VFV,VFW,dQdt)
      ...
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:),    intent(in), optional :: VF    !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:),    intent(in), optional :: VFU   !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:),    intent(in), optional :: VFV   !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:),    intent(in), optional :: VFW   !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               if (present(VFU).and.present(VFV).and.present(VFW)) then
               ...
               dQdt(i,j,k,2)=dQdt(i,j,k,2)-this%P(i,j,k)*((1.0_WP-VF_)*div+divp)
            end do
         end do
      end do
   ...
   subroutine get_primitive(this,VF)
      implicit none
      class(spcomp), intent(inout) :: this
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(in), optional :: VF !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      integer :: i,j,k
      ...
      do k=this%cfg%kmino_,this%cfg%kmaxo_; do j=this%cfg%jmino_,this%cfg%jmaxo_; do i=this%cfg%imino_,this%cfg%imaxo_
         this%RHO(i,j,k)=this%Q(i,j,k,1)
         if (present(VF)) this%RHO(i,j,k)=this%RHO(i,j,k)/VF(i,j,k)
         this%I(i,j,k)=this%Q(i,j,k,2)/this%Q(i,j,k,1)
         this%P(i,j,k)=this%getP(this%RHO(i,j,k),this%I(i,j,k))
         this%C(i,j,k)=this%getC(this%RHO(i,j,k),this%P(i,j,k))
      end do; end do; end do
      ! Get temperature
      if (associated(this%getT)) then
         do k=this%cfg%kmino_,this%cfg%kmaxo_; do j=this%cfg%jmino_,this%cfg%jmaxo_; do i=this%cfg%imino_,this%cfg%imaxo_
            this%T(i,j,k)=this%getT(this%RHO(i,j,k),this%P(i,j,k))
         end do; end do; end do
      end if
   ...
   subroutine get_info(this)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_; do j=this%cfg%jmin_,this%cfg%jmax_; do i=this%cfg%imin_,this%cfg%imax_
         this%RHOmin=min(this%RHOmin,this%RHO(i,j,k)); this%RHOmax=max(this%RHOmax,this%RHO(i,j,k))
