...
module geometry
   ...
   public :: geometry_init,xinj_dist,inj_norm_diam
   
   !> Single config
   type(config), public :: cfg
   
   ! Some work arrays for reading in the CAD data
   real(WP), parameter :: nozzle_end   =-0.09130000_WP
   real(WP), parameter :: xinj_dist    =-0.06623329454_WP
   real(WP), parameter :: inj_norm_diam=+0.016002_WP
...
contains
   ...
   subroutine geometry_init
      ...
      create_grid: block
         ...
         ! Create simple rectilinear grid - unstretched
         do i=1,nx+1
            x(i)=real(i-1,WP)/real(nx,WP)*Lx-0.5_WP*Lx
         end do
         call param_read('Inlet start',x0)
         x=x-x(nx+1)+x0
         do j=1,ny+1
         ...
         do k=cfg%kmino_,cfg%kmaxo_
            do j=cfg%jmino_,cfg%jmaxo_
               do i=cfg%imino_,cfg%imaxo_
                  ...
                  ! first nozzle wall and fill in liquid
