...
contains
   ...
   subroutine my_tagger(solver,lvl,time,tags_ptr)
      ...
      real(WP) :: lapU,lapV,lapW,u_sgs,Re,dist,rad
      integer :: i,j,k
      tags=tags_ptr
      ! Get mesh spacing
      ...
      do while (mfi%next())
         ...
         do k=bx%lo(3),bx%hi(3); do j=bx%lo(2),bx%hi(2); do i=bx%lo(1),bx%hi(1)
            ...
            Re=solver%rhoL*u_sgs*delta/viscL_mol
   ...
   subroutine dirichlet_velocity(solver,lvl,time,face,bx,comp,p)
      ...
      case (1)  ! Inflow in X-
         select case (comp)
          ...
          case ('Q')  ! Cell-centered: U=Uinlet, V=0, W=0
            do k=bx%lo(3),bx%hi(3); do j=bx%lo(2),bx%hi(2); do i=bx%lo(1),bx%hi(1)
               p(i,j,k,1)=Uinlet
               p(i,j,k,2)=0.0_WP
               p(i,j,k,3)=0.0_WP
   ...
   subroutine simulation_init()
      ...
      create_VFib: block
         ...
         call VFib%initialize(amr,name='VFib',ncomp=1,ng=fs%nover,interp=interp_const)
         call VFib%register()
