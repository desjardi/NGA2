module avl_trees
   ! https://rosettacode.org/wiki/AVL_tree#Fortran
   !  
   ! References:
   !
   ...
   ! Cast retreived values
   public :: int_cast,real_cast
   ...
   contains
      ...
      recursive subroutine free_the_nodes (p)
         ...
         if (associated (p)) then
            if (associated(p%left)) call free_the_nodes (p%left)
            if (associated(p%right)) call free_the_nodes (p%right)
            deallocate (p)
         end if
