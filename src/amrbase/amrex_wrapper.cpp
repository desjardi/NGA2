...
//-----------------------------------------------------------------------------
// Restriction operator to determine unique id's on coarser levels
//-----------------------------------------------------------------------------

extern "C" void amrmfab_restrict_unique_id(void *crse_mf_ptr,
                                           void *fine_mf_ptr,
                                           int *ref_ratio)
{
    auto *cmf = static_cast<amrex::MultiFab *>(crse_mf_ptr);
    ...
    amrex::IntVect ratio(AMREX_D_DECL(ref_ratio[0],
                                     ref_ratio[1],
                                     ref_ratio[2]));

    // --------------------------------------------
    // Gather all local fine FABs (no nested MFIter)
    // --------------------------------------------
    std::vector<amrex::Box> fine_boxes;
    std::vector<amrex::Array4<const amrex::Real>> fine_arrays;

    for (amrex::MFIter mfi_f(*fmf, amrex::TilingIfNotGPU()); mfi_f.isValid(); ++mfi_f)
    {
        const auto& fab = (*fmf)[mfi_f];
        fine_boxes.push_back(fab.box());
        fine_arrays.push_back(fab.const_array());
    }

    // --------------------------------------------
    // Coarse loop
    // --------------------------------------------
    for (amrex::MFIter mfi(*cmf, amrex::TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        const amrex::Box& cbox = mfi.tilebox();
        auto const& carr = cmf->array(mfi);

        // ✅ define fbox (this was missing)
        amrex::Box fbox = amrex::refine(cbox, ratio);

        amrex::LoopOnCpu(cbox,
        [=, &fine_boxes, &fine_arrays] (int i, int j, int k) noexcept
        {
            bool found    = false;
            bool conflict = false;
            int id_store  = 0;

            const auto& clo = cbox.smallEnd();
            const auto& flo = fbox.smallEnd();

            // ✅ correct mapping (this fixes your wrong IDs problem)
            int fi0 = (i - clo[0]) * ratio[0] + flo[0];
            int fj0 = (j - clo[1]) * ratio[1] + flo[1];
            int fk0 = (k - clo[2]) * ratio[2] + flo[2];

            for (int n = 0; n < fine_boxes.size(); ++n)
            {
                const auto& fbx  = fine_boxes[n];
                const auto& farr = fine_arrays[n];

                // Only consider overlapping region
                amrex::Box overlap = fbox & fbx;
                if (!overlap.ok()) continue;

                for (int kk = 0; kk < ratio[2]; ++kk)
                for (int jj = 0; jj < ratio[1]; ++jj)
                for (int ii = 0; ii < ratio[0]; ++ii)
                {
                    int fi = fi0 + ii;
                    int fj = fj0 + jj;
                    int fk = fk0 + kk;

                    // Check if inside this FAB
                    if (fi >= overlap.smallEnd(0) && fi <= overlap.bigEnd(0) &&
                        fj >= overlap.smallEnd(1) && fj <= overlap.bigEnd(1) &&
                        fk >= overlap.smallEnd(2) && fk <= overlap.bigEnd(2))
                    {
                        amrex::Real val = farr(fi,fj,fk,0);

                        if (val > 0.5)
                        {
                            int id_val = static_cast<int>(amrex::Math::round(val));
            ...
            if (!found || conflict)
                ...
                carr(i,j,k,0) = static_cast<amrex::Real>(id_store);
        });
    }
}