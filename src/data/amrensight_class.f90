!> amrensight class concept is defined here: given an amr config object and associated data,
...
contains
   ...
   subroutine initialize(this,amr,name)
      ...
      if (this%amr%amRoot) then
         ...
         if (.not.isdir('ensight/'//trim(this%name)//'/geometry')) &
         & call makedir('ensight/'//trim(this%name)//'/geometry')
         if (.not.isdir('ensight/'//trim(this%name)//'/AMRflag')) &
         & call makedir('ensight/'//trim(this%name)//'/AMRflag')
      end if
      
      ! Create casefile name for out first level - we'll use it to read in time
      casename='ensight/'//trim(this%name)//'/nga.case'
   ...
   subroutine write_case(this)
      ...
      type(scl), pointer :: my_scl
      type(vct), pointer :: my_vct
      character(len=str_medium) :: casename
      ...
      write(iunit,'(a)') 'scalar per element: 1 AMRflag AMRflag/AMRflag.******'
      my_scl=>this%first_scl
      do while (associated(my_scl))
   ...
   subroutine write_geom(this)
      ...
      use mpi_f08,          only: MPI_BCAST,MPI_INTEGER
      use amrex_amr_module, only: amrex_box,amrex_mfiter,amrex_multifab
      implicit none
      class(amrensight), intent(in) :: this
      integer :: iunit,ierr,lvl
      type(amrex_box)    :: bx
      type(amrex_mfiter) :: mfi
      character(len=str_medium) :: filename
      character(len=80) :: cbuff
      real(SP) :: rbuff
      integer :: ibuff,rank,nbox,i,j,k
      type(amrex_multifab), dimension(:), allocatable :: blank
      real(WP), dimension(:,:,:,:), contiguous, pointer :: myblank
      real(SP), dimension(:,:,:), allocatable :: spblank
      
      ! First write out geometry ==================================================
         ...
         do rank=0,this%amr%nproc-1
            if (rank.eq.this%amr%rank) then
               ...
               do while (mfi%next())
                  ...
                  write(cbuff,'("Level ",i0," - Box ",i0," - Rank ",i0)') lvl,nbox,rank; write(iunit) cbuff
                  ...
                  ! Mesh origin
                  rbuff=real(this%amr%xlo+bx%lo(1)*this%amr%dx(lvl),SP); write(iunit) rbuff
                  rbuff=real(this%amr%ylo+bx%lo(2)*this%amr%dy(lvl),SP); write(iunit) rbuff
                  rbuff=real(this%amr%zlo+bx%lo(3)*this%amr%dz(lvl),SP); write(iunit) rbuff
                  ! Mesh size
                  rbuff=real(this%amr%dx(lvl),SP); write(iunit) rbuff
                  rbuff=real(this%amr%dy(lvl),SP); write(iunit) rbuff
                  rbuff=real(this%amr%dz(lvl),SP); write(iunit) rbuff
               end do
               ! Close the file
               close(iunit)
            end if
      ...
      ! Then write out blanking info ==============================================
      ! Allocate blanking mfab array and populate with maximum level
      allocate(blank(0:this%amr%clvl()))
      do lvl=0,this%amr%clvl()
         call this%amr%mfab_build(lvl,blank(lvl),ncomp=1,nover=0)
         call this%amr%mfiter_build(lvl,mfi)
         do while (mfi%next())
            myblank=>blank(lvl)%dataptr(mfi)
            myblank=real(lvl,WP)
         end do
         call this%amr%mfiter_destroy(mfi)
      end do
      call this%amr%average_down(blank)
      
      ! Generate timestamped filename for blanking info
      filename='ensight/'//trim(this%name)//'/AMRflag/AMRflag.'
      write(filename(len_trim(filename)+1:len_trim(filename)+6),'(i6.6)') this%ntime
      ...
      if (this%amr%amRoot) then
         ...
         if (ierr.ne.0) call die('[amrensight write geom] Could not open file '//trim(filename))
         ! Write the header
         cbuff='AMRflag'; write(iunit) cbuff
         ! Close the file
         close(iunit)
      end if
         ...
         do rank=0,this%amr%nproc-1
            if (rank.eq.this%amr%rank) then
               ...
               do while (mfi%next())
                  ...
                  ! Allocate spblank to right size
                  allocate(spblank(bx%lo(1):bx%hi(1),bx%lo(2):bx%hi(2),bx%lo(3):bx%hi(3)))
                  ! Copy data from blanking multifab
                  myblank=>blank(lvl)%dataptr(mfi)
                  do k=bx%lo(3),bx%hi(3)
                     do j=bx%lo(2),bx%hi(2)
                        do i=bx%lo(1),bx%hi(1)
                           if (myblank(i,j,k,1).gt.lvl) then
                              spblank(i,j,k)=real(1,SP)
                           else
                              spblank(i,j,k)=real(0,SP)
                           end if
                        end do
                     end do
                  end do
      ...
      ! Deallocate blank mfabs
      do lvl=0,this%amr%clvl()
         call this%amr%mfab_destroy(blank(lvl))
      end do
      deallocate(blank)
      ! ===========================================================================
   ...
   subroutine write_scalar(this,my_scl)
      use precision,        only: SP
      use messager,         only: die
      use parallel,         only: info_mpiio,MPI_REAL_SP
      use mpi_f08,          only: MPI_BCAST,MPI_INTEGER
      ...
      character(len=str_medium) :: filename
      integer :: iunit,ierr,nbox,rank,ibuff,lvl
      character(len=80) :: cbuff
      type(amrex_box)    :: bx
      type(amrex_mfiter) :: mfi
      ...
      do lvl=0,this%amr%clvl()
         ...
         do rank=0,this%amr%nproc-1
            if (rank.eq.this%amr%rank) then
               ! Open the file
               open(newunit=iunit,file=trim(filename),form='unformatted',status='old',access='stream',position='append',iostat=ierr)
               if (ierr.ne.0) call die('[amrensight write scalar] Could not reopen file '//trim(filename))
               ! Loop through all boxes in mfiter
               do while (mfi%next())
                  ! Get box
                  bx=mfi%tilebox()
   ...
   subroutine write_vector(this,my_vct)
      use precision,        only: SP
      use messager,         only: die
      use parallel,         only: info_mpiio,MPI_REAL_SP
      use mpi_f08,          only: MPI_BCAST,MPI_INTEGER
   ...
   subroutine write_surf(this,surf)
      ...
      if (this%amr%amRoot) then
         ! Open the case file
         filename='ensight/'//trim(this%name)//'/'//trim(surf%name)//'.case'
   ...
   subroutine write_part(this,part)
      ...
      if (this%amr%amRoot) then
         ...
         ! Write all the geometry information
         write(iunit,'(a,/,a,/,/,a,/,a,/,a,/)') 'FORMAT','type: ensight gold','GEOMETRY','model: 1 geometry geometry/geometry.******','measured: 1 '//trim(part%name)//'/particle.******'
         ! Write the variables
         write(iunit,'(a)') 'VARIABLE'
