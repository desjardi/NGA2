...
module multiscalar_class
   ...
   !> Declare multiscalar solver constructor
   interface multiscalar
      procedure constructor
   end interface multiscalar
   
contains
   
   
   !> Default constructor for multiscalar solver
   function constructor(cfg,scheme,nscalar,name) result(self)
      use messager, only: die
      implicit none
      type(multiscalar) :: self
   ...
   subroutine add_bcond(this,name,type,locator,dir)
      ...
      if (present(dir)) then
         ...
         case default; call die('[multiscalar add_bcond] Unknown bcond direction')
         end select
      else
      ...
      case default
         call die('[multiscalar apply_bcond] Unknown bcond type')
      end select
   ...
   subroutine get_bcond(this,name,my_bc)
      ...
      if (.not.associated(my_bc)) call die('[multiscalar get_bcond] Boundary condition was not found')
